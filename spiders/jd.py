...
class JdSpider(scrapy.Spider):
    ...
    url = 'https://search.jd.com/Search?keyword=%s&enc=utf-8&qrst=1&rt=1&stop=1&vt=2&wq=%s&page=%d&s=1&click=0'
    next_url = 'https://search.jd.com/s_new.php?keyword=%s&enc=utf-8&qrst=1&rt=1&stop=1&vt=2&wq=%s&page=%d&s=27&scrolling=y&log_id=1551189598.97521&tpl=1_M&show_items=%s'
    ...
    def parse(self, response):
        ...
        self.page += 1
        print('*********',self.next_url % (self.keyword, self.keyword, self.page, ','.join(ids)))
    ...
    def info_parse(self, response):
        ...
        prev = response.xpath('//div[@class="detail"]/text()').extract()[1]
